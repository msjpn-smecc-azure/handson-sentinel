...
1. [Microsoft Sentinel]-[脅威管理]-[ブック] を開き、「テンプレート」タブへ移動

    <!-- ![](../images/ex07-401.png) -->

1. `Threat Intelligence` を選択、「テンプレートの表示」を選択

    <!-- ![](../images/ex07-402.png) -->

1. ブック保存先リージョン `Japan East` を指定して「保存」
...
1. 「Threat Intelligence ブック」の確認
    ...
    Sentinelにインポートされたインジケーターをタイプ別およびプロバイダー別に示す、事前に作成された視覚化がいくつか見つかります。新しいグラフを変更または追加するには、ページの上部にある [編集] ボタンを選択して、ブックの編集モードに入ります。
