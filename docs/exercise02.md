...
> [!CAUTION]  
> 実際にデータコネクタを接続すると、テナント上のデータが Microsoft Sentinel ワークスペースに取り込まれます。
> 取り込まれたデータは、Microsoft Sentinel ワークスペースのログの課金対象となるため、**不要なデータコネクタ, 予算が正しく計画されていない場合は接続しないように注意** してください。
...
1. [MicrosoftSentinel]-[コンテンツ管理]-[コンテンツハブ] を開く
    ...
    ![](../images/ex02/0101.png)

1. `Azure Activity` を検索して選択、「インストール」を実行

    ![](../images/ex02/0102.png)

1. [MicrosoftSentinel]-[構成]-[データコネクタ] を開く

    ![](../images/ex02/0103.png)

1. `Azure Activity` を選択、「コネクターページを開く」を選択

    ![](../images/ex02/0104.png)

1. 「セットアップ(Setup)」を下へスクロールして「構成(Configuration)」にある「[Azure Policy の割り当て]ウィザードの起動」を選択（Azureポータルへ遷移）

    ![](../images/ex02/0105.png)
...
1. ポリシーの割り当て
    ...
    1. 基本情報
        ...
        - 基本情報
            ...
            - バージョン: (最新のものを選択)
            - 割り当て名: `指定された Log Analytics ワークスペース (YOUR_LOG_ANALYTICS_WORKSPACE_NAME) にストリーミングするように Azure アクティビティ ログを構成します` (デフォルト入力されていますが、**送信先 Log Analytics ワークスペース名 がわかるような名前に変更**を推奨)

        ![](../images/ex02/0106a.png)

    1. パラメーター

        - プライマリ Log Analytics ワークスペース: (今回のハンズオン用に作成したワークスペース)

        ![](../images/ex02/0106b.png)
    ...
    1. マネージドID
        ...
        ![](../images/ex02/0106d.png)

    1. 非準拠メッセージ

        - コンプライアンス非対応のメッセージ:
            ```
            このサブスクリプションの Azure アクティビティ ログは、指定された Log Analytics ワークスペースに転送されていません。
            ```

        ![](../images/ex02/0106e.png)

    1. 確認および作成

        「作成」を選択

        ![](../images/ex02/0106f.png)

1. [コンテンツ管理]-[コンテンツハブ] を開き、`Azure Activity` を検索して選択、「管理」を開く

    ![](../images/ex02/0107.png)
...
> [!IMPORTANT]  
> この演習を実行するには、以下が必要です。<br />
> - Microsoft Defender for Cloud でいずれかの Defender プランが有効
<!-- > - ユーザーがサブスクリプションでセキュリティ リーダーロールを持っている必要があります。<br /> -->


1. [コンテンツ管理]-[コンテンツハブ] を開き、`Microsoft Defender for Cloud` を検索して選択、「インストール」を実行

    ![](../images/ex02/0201.png)

1. デプロイが完了したら「管理」を開く

    ![](../images/ex02/0202.png)

1. `Tenant-based Microsoft Defender for Cloud` のデータコネクタを選択、「コネクタページを開く」を押下（Azureポータルへ遷移）

    ![](../images/ex02/0203.png)

1. 「構成」の「接続」を選択（接続できていない場合のみ。接続済みの場合は確認のみで対応不要）

    ![](../images/ex02/0204.png)
